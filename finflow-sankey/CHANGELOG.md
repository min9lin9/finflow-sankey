...
### Changed
- Moved the package project under `finflow-sankey/` so it can coexist with other repositories in the same workspace.
- Reduced duplicated rendering/template code without changing public APIs.
- Updated GitHub workflows to run from the nested package directory.

### Fixed
- Added the missing Excel writer test dependency for CI.
- Kept the package compatible with Python 3.9 in the refactored income statement helpers.
