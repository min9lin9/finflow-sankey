...
class CashFlowStatementTemplate(StatementTemplate):
    ...
    def build(self, df: pl.DataFrame, **kwargs: Any) -> FinancialGraph:
        ...
        edge_specs = [(section, "beginning_cash", section) for section in flow_sections]
        edge_specs += [(section, section, "ending_cash") for section in flow_sections]

        for section, source, target in edge_specs:
            amount = section_info.get(section, {}).get("amount", 0.0)
            accounts = _section_accounts(section)
            if amount == 0.0:
                continue
